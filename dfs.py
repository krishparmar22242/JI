# File: dfs_tsp_visual_trace.py

import matplotlib.pyplot as plt
import networkx as nx

# --- Visualization Functions (Identical to BFS file) ---
...
def visualize_final_path(graph_data, path, cost):
    ...
    if not path:
        print("DFS: No solution path found to visualize.")
        return
    visualize_path(graph_data, path, cost, "DFS Final Optimal Path")
...
# --- DFS Algorithm with Visual Tracing ---

def dfs_tsp_visual_trace(graph, start_node):
    """Solves TSP using DFS with step-by-step visual tracing."""
    stack = [([start_node], 0)]  # Use a list as a stack
    min_cost = float('inf')
    best_path = None
    num_nodes = len(graph)
    step = 0
    ...
    while stack:
        step += 1
        current_path, current_cost = stack.pop() # LIFO behavior
...
if __name__ == '__main__':
    ...
    # 2. Run the DFS algorithm with visual tracing
    dfs_path, dfs_cost = dfs_tsp_visual_trace(graph_data, start_node)

    # 3. Show the final result
    print(f"DFS Search Complete. Final Path: {dfs_path}, Cost: {dfs_cost}")
    visualize_final_path(graph_data, dfs_path, dfs_cost)